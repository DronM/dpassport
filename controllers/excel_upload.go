...
func convertExcelFile(baseDir string, file multipart.File) (*os.File, error) {
	fl_id := genUniqID(10)
	txt_fn := baseDir + "/" + CACHE_PATH + fl_id +".txt"
	excel_fn := baseDir + "/" + CACHE_PATH + fl_id+".xlsx"
...
//Добавляет ошибочную строку в Excel и текст ошибки + к ошибкам
//header []string, 
func add_error_to_xls(values []string, errExcelSheetName *string, errExcelRow *int, errStr *string, errorExcelFile *excelize.File, err string) {
	if *errStr != "" {
		*errStr += " "
	}
	*errStr += err
	for i, col_v := range values {
		cell := getExcelCellRowForIndex(i)
		errorExcelFile.SetCellValue(*errExcelSheetName, cell + fmt.Sprintf("%d", *errExcelRow), col_v)
	}
	*errExcelRow++
}

func uploadStudyDocuments(dStore *pgds.PgProvider, log logger.Logger, baseDir string, file multipart.File, colMap columnMap, userID int64, userDescr, userRole, operationID string, companyID int64) {
	defer file.Close()
	//db connect
	...
	//convert file
	txt_file, err := convertExcelFile(baseDir, file)	
	if err != nil {
	...
	param_cnt := 20 //21 columns, 20 parameters
	...
	for doc := range extractStudyDocuments(conn, txt_file, colMap, 0, true) {        	
		if doc.Error != nil {
			add_error_to_xls(doc.Values, &error_excel_sheet_name, &error_excel_row, &err_str, error_excel_f, doc.Error.Error())
			continue
		...
		if doc.Document.Snils.GetValue() == "" {
			add_error_to_xls(doc.Values, &error_excel_sheet_name, &error_excel_row, &err_str, error_excel_f,
				fmt.Sprintf("Строка: %d, не задан СНИЛС!", doc_cnt+1))
		...
		//Проверить поля-справочники: StudyType, Profession, Qualification
		study_type_found := false
		profession_found := false
		qualification_found := false
		if err := conn.QueryRow(context.Background(), 
			`SELECT
				coalesce((SELECT TRUE FROM study_types WHERE name = $1), FALSE) AS study_type_found,
				coalesce((SELECT TRUE FROM professions WHERE name = $2), FALSE) AS profession_found,
				coalesce((SELECT TRUE FROM qualifications WHERE name = $3), FALSE) AS qualification_found`,
			doc.Document.Study_type, doc.Document.Profession, doc.Document.Qualification_name).Scan(&study_type_found,
			&profession_found, &qualification_found); err != nil {
			
			conn.Exec(context.Background(), `ROLLBACK`)
			endUserOperationWithError(log, conn, userID, operationID, errors.New(fmt.Sprintf("References check: %v", err)))
		}
		if !study_type_found {
			add_error_to_xls(doc.Values, &error_excel_sheet_name, &error_excel_row, &err_str, error_excel_f,
				fmt.Sprintf("Строка: %d, вид обучения '%s' не найден в справочнике!",
					doc_cnt+1, doc.Document.Study_type.GetValue()))
			continue
		
		}
		if !profession_found {
			add_error_to_xls(doc.Values, &error_excel_sheet_name, &error_excel_row, &err_str, error_excel_f,
				fmt.Sprintf("Строка: %d, профессия '%s' не найдена в справочнике!",
					doc_cnt+1, doc.Document.Profession.GetValue()))		
		}
		if !qualification_found {
			add_error_to_xls(doc.Values, &error_excel_sheet_name, &error_excel_row, &err_str, error_excel_f,
				fmt.Sprintf("Строка: %d, квалификация '%s' не найдена в справочнике!",
					doc_cnt+1, doc.Document.Qualification_name.GetValue()))		
		}
		...
		doc_ins_q+= fmt.Sprintf("('upload', $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d, $%d)",
			...
			doc_ins_param_cnt + 20,
		)
		doc_ins_param_cnt+= param_cnt
		doc_ins_params = append(doc_ins_params, doc.Document.Company_id)
		doc_ins_params = append(doc_ins_params, doc.Document.User_id) //всегда NULL
		...
		doc_ins_params = append(doc_ins_params, userID)
		doc_cnt_ins++
	...
	uploadDocumentEnd(conn, log, "StudyDocument", doc_cnt, err_str, error_excel_f, userID, companyID, userRole, operationID)
}
			...
			if ind > 0 {
				if !parse {
					...
					for i, arr_v := range arr {					
						...
						col_tp := ""
						col_descr := ""
						for _, col_m := range colMap {
							if col_m.Ind == i {
								col_tp = col_m.DataType
								col_descr = col_m.Descr
								break
							}
						}
						if col_tp == "DateTimeTZ" || col_tp == "DateTime" || col_tp == "Date" {
							if col_d, err := parseExcelDateVal(arr_v, col_descr); err == nil {
								excel_doc.Values[i] = col_d.Format("02/01/2006")
							}else{
...
//
func uploadStudyDocumentRegisters(dStore *pgds.PgProvider, log logger.Logger, baseDir string, file multipart.File, colMap columnMap, userID int64, userDescr, userRole, operationID string, companyID int64) {
	defer file.Close()
	//db connect
	var conn_id pgds.ServerID
	var pool_conn *pgxpool.Conn
	...
	param_cnt := 5 //6 cols, 5 params
	query_header := `INSERT INTO study_document_registers
		(issue_date, name, study_form, company_id, create_type, create_user_id)
		VALUES `
	...
	for doc := range extractStudyDocumentRegisters(conn, txt_file, colMap, 0, true) {        	
		...
		//Проверить поля-справочники: StudyType, Profession, Qualification
		study_form_found := false
		if err := conn.QueryRow(context.Background(), 
			`SELECT
				coalesce((SELECT TRUE FROM study_forms WHERE name = $1), FALSE) AS study_form_found`,
			doc.Document.Study_form).Scan(&study_form_found); err != nil {
		...
		if !study_form_found {
			add_error_to_xls(doc.Values, &error_excel_sheet_name, &error_excel_row, &err_str, error_excel_f,
				fmt.Sprintf("Строка: %d, форма обучения '%s' не найдена в справочнике!",
					doc_cnt+1, doc.Document.Study_form.GetValue()))
			continue
		...
		doc_ins_q+= fmt.Sprintf("($%d, $%d, $%d, $%d, 'upload', $%d)", doc_ins_param_cnt+1, doc_ins_param_cnt+2, doc_ins_param_cnt+3, doc_ins_param_cnt+4, doc_ins_param_cnt+5)
		doc_ins_param_cnt+= param_cnt
		doc_ins_params = append(doc_ins_params, doc.Document.Issue_date)
		doc_cnt_ins++
		doc_ins_params = append(doc_ins_params, doc.Document.Name)
		doc_cnt_ins++
		doc_ins_params = append(doc_ins_params, doc.Document.Study_form)
		doc_cnt_ins++
	...
	uploadDocumentEnd(conn, log, "StudyDocumentRegister", doc_cnt, err_str, error_excel_f, userID, companyID, userRole, operationID)
}
...
func uploadDocumentEnd(conn *pgx.Conn, log logger.Logger, docType string, docCnt int, errTxt string, errorExcel *excelize.File, userID, companyID int64, userRoleID, operationID string) {
	if _, err := conn.Exec(context.Background(), `COMMIT`); err != nil {
		endUserOperationWithError(log, conn, userID, operationID, err)
		return
	}
	comment_text := fmt.Sprintf("Загружено документов: %d", docCnt)
	if errTxt !="" && errorExcel != nil {
		var b bytes.Buffer
	...
	//one event
	EmitEvent(conn, fmt.Sprintf("%s.insert", docType), 0)
	//+ special events
	if userRoleID == "client_admin1" || userRoleID == "client_admin2" {
		EmitEvent(conn, fmt.Sprintf("%s.insert_comp_%d", docType, companyID), 0)
	}	
	if userRoleID == "client_admin2" {
		EmitEvent(conn, fmt.Sprintf("%s.insert_usr_%d", docType, userID), 0)
	}	
}
...
func parseExcelDateVal(excelStr, colDescr string) (time.Time, error) {
	...
	if len(col_v) == 10 && strings.IndexFunc(col_v[0:4], isNotDigit) == -1 && col_v[4:5] == "/" && col_v[7:8] == "/" {
		...
		return time.Time{}, errors.New("Неизвестный формат даты: "+col_v)
	}
...
func delExcelTemplate(baseDir string, xlsFileName string) error {
	xls_file_path := baseDir + "/" + CACHE_PATH + xlsFileName
...
func getExcelTemplate(app osbe.Applicationer, sock socket.ClientSocketer, resp *response.Response, xlsFileName string, constID string) error {
	xls_file_path := app.GetBaseDir() + "/" + CACHE_PATH + xlsFileName
	if !view.FileExists(xls_file_path) {
		//db connect
...
func getExcelError(app osbe.Applicationer, sock socket.ClientSocketer, resp *response.Response, xlsFileName string, operaionID string, userID int64) error {
	...
	if err := conn.QueryRow(context.Background(),
		...
		return osbe.NewPublicMethodError(response.RESP_ER_INTERNAL, fmt.Sprintf("getExcelError conn.QueryRow(): %v",err))
	}

	sock_http, ok := sock.(*httpSrv.HTTPSocket)
	if !ok {
		return osbe.NewPublicMethodError(response.RESP_ER_INTERNAL, "getExcelError sock must be *httpSrv.HTTPSocket")
	}
...
//fmt.Println("userID=", userID, "companyID=", companyID)
	if roleID != "admin" {
		//Нужны проверки
		allowed:= false;
		if err := conn.QueryRow(context.Background(), 
